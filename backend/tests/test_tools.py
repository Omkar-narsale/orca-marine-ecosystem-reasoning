...
from backend.app.services.incois.parser import parse_incois_wave_record
...
def test_compile_evidence_tool():
    test_rec = parse_incois_wave_record(lat=19.3, lon=72.5, wave_height_m=3.5)
    ev_res = asyncio.run(compile_evidence_tool.execute(records=[test_rec]))
